...
class Node {
  ...
  Node(float _len, float _size, float _rotRange, int _level) {
    ...
    if (level == 0 ) rot = 0;
    windFactor = random(0.2, 0.7);
    doesBloom = false;
    if (level >= leafLevel && random(1) < leafChance) doesBloom = true;
    bloomSize = random(bloomSizeAverage*0.2, bloomSizeAverage*1.4);
    leafRot = radians(random(-180, 180));
    flowerScaleT = random(0.8, 1.8);
  ...
  void randomizeColor() {
    branchColor = color(branchHue, random(130, 255), random(100, 200));
    leafColor = color(leafHue, leafSat, random(100, 255));
    flowerBright = random(150, 255);
